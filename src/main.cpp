...
bool CTransaction::ConnectInputs(CTxDB& txdb, MapPrevTx inputs, map<uint256, CTxIndex>& mapTestPool, const CDiskTxPos& posThisTx, const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (!IsCoinStake())
        {
            ...
            if (nTxFee < GetMinFee(1, GMF_BLOCK, 0))
                return fBlock? DoS(100, error("ConnectInputs() : %s not paying required fee=%s, paid=%s", GetHash().ToString().substr(0,10).c_str(), FormatMoney(GetMinFee(1, GMF_BLOCK, 0)).c_str(), FormatMoney(nTxFee).c_str())) : false;
        ...
        else
        {
            ...
            printf("END: nStakeReward = %i , CoinAge = %lu \n", nStakeReward, nCoinAge);
        }
    }
    return true;
}
...
uint64_t CTransaction::GetCoinAge(CTxDB& txdb, uint64_t nCoinAge, bool byValue) const
{
