...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    // Wallet comments
    CWalletTx wtx;
    if (request.params.size() > 2 && request.params[2].type() != UniValue::VNULL && !request.params[2].get_str().empty())
        wtx.mapValue["comment"] = request.params[2].get_str();
    if (request.params.size() > 3 && request.params[3].type() != UniValue::VNULL && !request.params[3].get_str().empty())
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4 && request.params[4].type() != UniValue::VNULL && !request.params[4].get_str().empty())
        wtx.mapValue["comment"] = request.params[4].get_str();
    if (request.params.size() > 5 && request.params[5].type() != UniValue::VNULL && !request.params[5].get_str().empty())
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in ECC received by the address\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    int64_t* pnSleepTime = new int64_t(request.params[1].get_int64());
...
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1 || AmountFromValue(request.params[0]) < MIN_TX_FEE(GetTime()))
        throw runtime_error(
            "settxfee <amount>\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total confirmed balance of the wallet in ECC\n"
            "  \"unconfirmed_balance\": xxx,   (numeric) the total unconfirmed balance of the wallet in ECC\n"
            "  \"immature_balance\": xxxxxx,   (numeric) the total immature balance of the wallet in ECC\n"
            "  \"txcount\": xxxxxxx,           (numeric) the total number of transactions in the wallet\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since Unix epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee configuration, set in ECC/kB\n"
    ...
    obj.push_back(Pair("is encrypted", (pwallet->IsCrypted())));
    if (pwalletMain->IsCrypted())
