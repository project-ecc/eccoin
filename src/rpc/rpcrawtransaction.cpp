...
void TxToJSON(const CTransaction &tx, const uint256 hashBlock, UniValue &entry)
{
    ...
    if (!hashBlock.IsNull())
    {
        ...
        CBlockIndex *pindex = g_chainman.LookupBlockIndex(hashBlock);
        if (pindex)
        {
...
UniValue gettxoutproof(const UniValue &params, bool fHelp)
{
    ...
    for (unsigned int idx = 0; idx < txids.size(); idx++)
    {
        ...
        if (setTxids.count(hash))
            throw JSONRPCError(
                RPC_INVALID_PARAMETER, std::string("Invalid parameter, duplicated txid: ") + txid.get_str());
    ...
    if (params.size() > 1)
    {
        ...
        pblockindex = g_chainman.mapBlockIndex[hashBlock];
    }
    else
    {
        LOCK(cs_main);
        ...
        if (coin && !coin->IsSpent() && coin->nHeight > 0 &&
            coin->nHeight <= (unsigned int)g_chainman.chainActive.Height())
        {
            pblockindex = g_chainman.chainActive[coin->nHeight];
        }
    }
...
UniValue verifytxoutproof(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "[\"txid\"]      (array, std::strings) The txid(s) which the proof commits to, or empty array if the proof "
            "is invalid\n");
    ...
    auto *pindex = g_chainman.LookupBlockIndex(merkleBlock.header.GetHash());

    {
        LOCK(cs_main);
        if (!pindex || !g_chainman.chainActive.Contains(pindex))
...
UniValue createrawtransaction(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the ECC address, the numeric value "
            "(can be string) is the " +
            CURRENCY_UNIT +
            " amount\n"
...
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error("decoderawtransaction \"hexstring\"\n"
...
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error("decodescript \"hex\"\n"
...
UniValue signrawtransaction(const UniValue &params, bool fHelp)
{
    ...
    if (params.size() > 3 && !params[3].isNull())
    {
        static std::map<std::string, int> mapSigHashValues =
            boost::assign::map_list_of(std::string("ALL"), int(SIGHASH_ALL))(std::string("ALL|ANYONECANPAY"),
                int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))(std::string("NONE"), int(SIGHASH_NONE))(
                std::string("NONE|ANYONECANPAY"), int(SIGHASH_NONE | SIGHASH_ANYONECANPAY))(std::string("SINGLE"),
                int(SIGHASH_SINGLE))(std::string("SINGLE|ANYONECANPAY"), int(SIGHASH_SINGLE | SIGHASH_ANYONECANPAY));
...
UniValue sendrawtransaction(const UniValue &params, bool fHelp)
{
    ...
    CTransaction ttx;
    if (!DecodeHexTx(ttx, params[0].get_str()))
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TX decode failed");
    CTransactionRef tx(MakeTransactionRef(std::move(ttx)));
    ...
    {
        CoinAccessor existingCoin(view, COutPoint(txid, o));
    ...
    if (!fHaveMempool && !fHaveChain)
    {
        ...
        bool fMissingInputs;
        if (!AcceptToMemoryPool(mempool, state, std::move(tx), false, &fMissingInputs, false, !fOverrideFees))
        {
            if (state.IsInvalid())
            {
                throw JSONRPCError(
