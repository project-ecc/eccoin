...
UniValue getgenerate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error("getgenerate\n"
...
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error("getgeneratepos\n"
                                 "\nReturn if the server is set to generate pos coins or not. The default is false.\n"
                                 "It is set with the command line argument -staking (or " +
                                 std::string(CONF_FILENAME) +
                                 " setting staking)\n"
                                 ...
                                 "\nExamples:\n" +
                                 HelpExampleCli("getgeneratepos", "") + HelpExampleRpc("getgeneratepos", ""));
    ...
    bool fProofOfStake)
{
    ...
    while (nHeight < nHeightEnd)
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(
            CreateNewBlock(pwalletMain, coinbaseScript->reserveScript, fProofOfStake));
...
UniValue generatepos(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error("generate numblocks ( maxtries )\n"
                                 "\nMine up to numblocks blocks immediately (before the RPC call returns)\n"
...
UniValue generatetoaddress(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
    {
        throw std::runtime_error("generatetoaddress numblocks  address\n"
...
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error("setgenerate \n"
...
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error("setgeneratepos \n"
...
UniValue getmininginfo(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"generate\": true|false     (boolean) If the pow generation is on or off (see getgenerate or "
            "setgenerate calls)\n"
            "  \"generatepos\": true|false  (boolean) If the pos generation is on or off (see getgeneratepos or "
            "setgeneratepos calls)\n"
            "}\n"
            "\nExamples:\n" +
...
UniValue getposblocktemplate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getblocktemplate ( \"jsonrequestobject\" )\n"
...
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error("submitblock \"hexdata\" ( \"jsonparametersobject\" )\n"
                                 ...
                                 "    {\n"
                                 "      \"workid\" : \"id\"    (string, optional) if the server provided a workid, it "
                                 "MUST be included with submissions\n"
                                 "    }\n"
                                 "\nResult:\n"
                                 "\nExamples:\n" +
...
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error("estimatefee nblocks\n"
