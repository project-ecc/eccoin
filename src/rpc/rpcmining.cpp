...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        const Value& oparam = params[0].get_obj();
        ...
        if (aClientRules.isArray()) {
            for (unsigned int i = 0; i < aClientRules.size(); ++i) {
                const Value& v = aClientRules[i];
                setClientRules.insert(v.get_str());
            }
        } else {
            // NOTE: It is important that this NOT be read if versionbits is supported
            const Value& uvMaxVersion = Value.find_value(oparam, "maxversion");
    ...
    Value aCaps(Value::Object); aCaps.push_back("proposal");

    Value transactions(Value::Array);
    ...
    Value aux(Value::Object);
    aux.push_back(Pair("flags", HexStr(COINBASE_FLAGS.begin(), COINBASE_FLAGS.end())));

    arith_uint256 hashTarget = arith_uint256().SetCompact(pblock->nBits);

    Value aMutable(Value::Array);
    ...
    Value aRules(Value::Array);
    Value vbavailable(Value::Object);
