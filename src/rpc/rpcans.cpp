...
*/
static CAmount CalcAnsFeeFromMonths(uint8_t months) { return (months * 50) * COIN; }
static void CreatePayment(const CTxDestination &address,
...
static void CreateANStransaction(CServiceTransaction &stxnew,
    CWalletTx &wtxnew,
...
UniValue registerans(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
    {
        throw std::runtime_error("getansaddress \"eccaddress\" \"username\" \n"
    ...
    while (i < strUsername.length())
    {
        ...
        {
            throw JSONRPCError(RPC_INVALID_PARAMS, "Username contains invalid charactar. All alphanumeric characters "
                                                   "EXCEPT for zero, capital i, capital o, and lowercase L are "
                                                   "allowed");
        }
        i++;
    }
    EnsureWalletIsUnlocked();
    // check to make sure we dont already have an PTR record for that address, this prevents people from accidently
    // buying a second name when they already have one
    ...
    // TODO : have user enter coin amount to pay for ans address, right now it is set to the default 1 month and cant be
    // changed
    ...
    wtx.tx->UpdateHash();
    // paymentReferenceHash of service transaction must be set AFTER rehashing the payment hash as the rehash of the
    // payment hash will include the hash of the service transaction
    ...
    if (!pwalletMain->CommitTransactionForService(stx, addr, g_connman.get()))
    {
        throw JSONRPCError(RPC_WALLET_ERROR,
            "Error: The service transaction was rejected! This might happen for a variety of reasons.");
    }
    std::string responseMessage = "The user name " + strUsername + " has been assigned to the address " + strAddress +
                                  ""
                                  " with PaymentHash = " +
                                  wtx.tx->GetHash().GetHex() + " and  ServiceHash = " + stx.GetHash().GetHex();
...
UniValue renewans(const UniValue &params, bool fHelp)
{
    ...
    {
        throw std::runtime_error("renewans \"eccaddress\" \"username\" \"code\" \n"
    ...
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + " to send. eg 0.1\n"
                            "3. \"code\"     (string, required) The verification code for the ans username supplied in "
                            "the first param.\n"
                            "\nResult:\n"
