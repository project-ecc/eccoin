            ...
            if (wtxIn.hashBlock != 0)
            {
                if (mapBlockIndex.count(wtxIn.hashBlock))
                {
                    ...
                    unsigned int latestEntry = 0;
                    {
                        ...
                        CWallet::TxItems txOrdered = OrderedTxItems(acentries);
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    ...
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                    pwalletMain->NotifyTransactionChanged(pwalletMain, wtx.GetHash(), CT_UPDATED);
                }
            }
            else
            {
...
void CWallet::ResendWalletTransactions(bool fForce)
{
    if (!fForce)
    {
        ...
        if (nBestTimeReceived < nLastTime)
            return;
        nLastTime = GetTime();
    }
    ...
    CTxDB txdb("r");
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (fForce || nBestTimeReceived - (int64_t)wtx.nTimeReceived > 5 * 60)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CTransaction& txNew)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown; n++)
        {
            ...
            if (CheckStakeKernelHashScrypt(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake))
            {
               // Found a kernel
    ...
    // Calculate coin age reward
    {
        ...
        nCredit += GetProofOfStakeReward(nCoinAge, nBits, txNew.nTime, pIndex0->nHeight, 0);
    }
    ...
    while(true)
    {
        ...
        if (nMinFee < txNew.GetMinFee(1, GMF_BLOCK, 0) - MIN_TX_FEE)
        {
            nMinFee = txNew.GetMinFee(1, GMF_BLOCK, 0) - MIN_TX_FEE;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (!SelectCoinsSimple(nBalance, txNew.nTime, nCoinbaseMaturity + 10, setCoins, nValueIn))
        return false;
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nReward = GetProofOfStakeReward(nCoinAge, nBits, txNew.nTime, pindexPrev->nHeight, nFees);
        if (nReward <= 0)
            return false;
...
bool CWallet::CreateScryptCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CTransaction& txNew)
{
    // The following split & combine thresholds are important to security
    // Should not be adjusted if you don't understand the consequences
    static unsigned int nStakeSplitAge = (60 * 60 * 24 * 30);
