...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CTransaction& txNew)
{
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nCreditReward = GetProofOfStakeReward(txNew.GetCoinAge(txdb, nCoinAge,true), pIndex0->nHeight);
        printf("nCreditReward create=%I64d \n", nCreditReward);
...
{
    {
        ...
        if (block.IsProofOfWork() && mapWallet.count(block.vtx[0].GetHash()))
        {
            ...
            printf("    mine:  %d  %d  %I64d ", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
        }
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            CWalletTx& wtx = mapWallet[block.vtx[1].GetHash()];
            printf("    stake: %d  %d  %I64d ", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
         }
...
{
    {
        ...
        printf("CWallet::NewKeyPool wrote %I64d new keys\n", nKeys);
    }
    return true;
}
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            printf("keypool added key %I64d , size=%" PRIszu "\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
