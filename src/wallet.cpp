...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CTransaction& txNew)
{
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nCreditReward = GetProofOfStakeReward(nCoinAge, nBits, txNew.nTime, pIndex0->nHeight,0);
        nCredit = nCredit + (nCreditReward / 2);
    }
    ...
    while(true)
    {
        ...
        if (nMinFee < txNew.GetMinFee(1,false,GMF_BLOCK,0) - MIN_TX_FEE)
        {
            nMinFee = txNew.GetMinFee(1,false,GMF_BLOCK,0) - MIN_TX_FEE;
