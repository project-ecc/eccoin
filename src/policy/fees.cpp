...
FeeFilterRounder::FeeFilterRounder(const CFeeRate &minIncrementalFee) {
    ...
    feeset.insert(0);
    for (double bucketBoundary = minFeeLimit; bucketBoundary <= double(MAX_FEERATE); bucketBoundary *= FEE_SPACING)
    {
        feeset.insert(int64_t(bucketBoundary));
    }
}

CAmount FeeFilterRounder::round(const CAmount currentMinFee) {
