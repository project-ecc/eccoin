...
bool CAnsZone::existsRecord(CAnsKey key) { return g_ans->ExistsEntry(key); }
...
bool CAnsZone::addRecord(AnsRecordTypes recordType, std::string key, CAnsRecord &value)
{
    ...
    case A_RECORD:
    {
        ...
        // add the record to to record set. since its a set if a record with that key already exists, nothing will be
        // changed
