...
CServiceTransaction::CServiceTransaction()
    : nVersion(CServiceTransaction::CURRENT_VERSION), nServiceId(0), nTime(GetAdjustedTime()), nOpCode(0),
      nExpireTime(0), vdata(), paymentReferenceHash()
{ //, securityHash() {
    paymentReferenceHash.SetNull();
    //    securityHash.SetNull();
}

CServiceTransaction::CServiceTransaction(const CServiceTransaction &tx)
    : nVersion(tx.nVersion), nServiceId(tx.nServiceId), nTime(tx.nTime),
      //, securityHash(tx.securityHash) {
      nOpCode(tx.nOpCode), nExpireTime(tx.nExpireTime), vdata(tx.vdata), paymentReferenceHash(tx.paymentReferenceHash)
{
    UpdateHash();
}
...
std::string CServiceTransaction::ToString() const
{
    ...
    str += strprintf("CServiceTransaction(hash=%s, ver=%d, nServiceId=%u, nTime=%u, nOpCode=%u, nExpireTime=%u, "
                     "vdata=%s, paymentReferenceHash=%s)\n",
        GetHash().ToString().substr(0, 10), nVersion, nServiceId, nTime, nOpCode, nExpireTime, datastr.c_str(),
