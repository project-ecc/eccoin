...
bool CTxMemPool::UpdateForDescendants(txiter updateIt, int maxDescendantsToVisit, cacheMap &cachedDescendants, const std::set<uint256> &setExclude)
{
    ...
    while (!stageEntries.empty()) {
        ...
        for (auto const childEntry: setChildren) {
            cacheMap::iterator cacheIt = cachedDescendants.find(childEntry);
            if (cacheIt != cachedDescendants.end()) {
                // We've already calculated this one, just add the entries for this set
                // but don't traverse again.
                for (auto const cacheEntry: cacheIt->second) {
    ...
    for (auto cit: setAllDescendants) {
...
bool CTxMemPool::CalculateMemPoolAncestors(const CTxMemPoolEntry &entry, setEntries &setAncestors, uint64_t limitAncestorCount, uint64_t limitAncestorSize, uint64_t limitDescendantCount, uint64_t limitDescendantSize, std::string &errString, bool fSearchForParents /* = true */)
{
    ...
    while (!parentHashes.empty()) {
        ...
        for (auto const& phash: setMemPoolParents) {
...
void CTxMemPool::UpdateChildrenForRemoval(txiter it)
{
    ...
    for (auto updateIt: setMemPoolChildren) {
...
void CTxMemPool::UpdateForRemoveFromMempool(const setEntries &entriesToRemove, bool updateDescendants)
{
    ...
    if (updateDescendants)
    {
        ...
        for (txiter removeIt : entriesToRemove)
        {
            ...
            CAmount modifyFee = -1 * removeIt->GetModifiedFee();
...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool fCurrentEstimate)
{
    ...
    for (auto const& phash: setParentTransactions) {
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    for (auto const& txin: it->GetTx().vin)
...
void CTxMemPool::remove(const CTransaction &origTx, std::list<CTransaction>& removed, bool fRecursive, MemPoolRemovalReason reason)
{
...
void CTxMemPool::removeForReorg(const CCoinsViewCache *pcoins, unsigned int nMemPoolHeight, int flags)
{
    ...
    for (auto const& tx: transactionsToRemove) {
