...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ECC unit input in lnReqAmount, we generate the URI with ECC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ECC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
