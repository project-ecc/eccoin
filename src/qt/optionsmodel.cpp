...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ECC);
...
*/
    if (!settings.contains("strDataDir"))
        settings.setValue("strDataDir", QString(GetDefaultDataDir().string().c_str()));
...
void OptionsModel::Reset()
{
    ...
    QString dataDir = QString(GetDefaultDataDir().string().c_str());
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        if (settingsVersion < 130000 && settings.contains("nDatabaseCache") && settings.value("nDatabaseCache").toLongLong() == 100)
            settings.setValue("nDatabaseCache", (qint64)450);
