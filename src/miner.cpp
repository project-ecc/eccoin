...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64_t nFees = 0;
    {
        ...
        u_int64_t nBlockSize = 1000;
        u_int64_t nBlockTx = 0;
...
CBlock* CreateNewScryptBlock(CWallet* pwallet, bool fProofOfStake)
{
    CReserveKey reservekey(pwallet);
    ...
    if (fProofOfStake)  // attempt to find a coinstake
    {
        ...
        {
            // printf(">>> OK1\n");
            if (pwallet->CreateScryptCoinStake(*pwallet, pblock->nBits, nSearchTime-nLastCoinStakeSearchTime, txCoinStake))
            {
                if (txCoinStake.nTime >= max(pindexPrev->GetMedianTimePast()+1, pindexPrev->GetBlockTime() - nMaxClockDrift))
                {   // make sure coinstake would meet timestamp protocol
                    // as it would be the same as the block timestamp
    ...
    int64_t nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // ppcoin: simplify transaction fee - allow free = false
            int64_t nMinFee = tx.GetMinScryptFee(nBlockSize, false, GMF_BLOCK);
...
bool CheckWorkForScrypt(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    uint256 hash = pblock->GetHash();
    uint256 hashTarget = CBigNum().SetCompact(pblock->nBits).getuint256();
...
void static ThreadScryptMiner(void* parg);
