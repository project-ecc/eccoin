...
BOOST_AUTO_TEST_CASE(cnode_simple_test)
{
    ...
    std::unique_ptr<CNode> pnode1(new CNode(id++, NODE_NETWORK, hSocket, addr, 0, 0, pszDest, fInboundIn));
    BOOST_CHECK(pnode1->fInbound == false);
    BOOST_CHECK(pnode1->fFeeler == false);

    fInboundIn = true;
    std::unique_ptr<CNode> pnode2(new CNode(id++, NODE_NETWORK, hSocket, addr, 1, 1, pszDest, fInboundIn));
